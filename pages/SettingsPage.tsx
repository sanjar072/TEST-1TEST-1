...
import { Trash2, List, Users, Database, Download, Upload, AlertTriangle, CheckCircle } from 'lucide-react';
// @ts-ignore
...
export const SettingsPage: React.FC<SettingsProps> = (props) => {
  const [activeTab, setActiveTab] = useState<'org' | 'details' | 'db'>('org');
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="flex border-b border-gray-200 overflow-x-auto">
          ...
          <TabButton 
            ...
            label="База Данных (Backup)" 
            icon={<Database size={18}/>} 
          />
        </div>
        <div className="p-6">
          {activeTab === 'org' && <OrganizationTab {...props} />}
...
// --- 3. Database Tab (Import/Export) ---

const DatabaseTab: React.FC<SettingsProps> = () => {
...
// --- 2. Details Dictionary Tab (Global Types) ---

const DetailsDictTab: React.FC<SettingsProps> = ({ details, setDetails }) => {
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {details.map(d => (
                    <div key={d.id} className="flex justify-between items-center p-3 bg-gray-50 rounded border hover:border-gray-300 transition-colors">
                        ...
                        <button onClick={() => setDetails(details.filter(x => x.id !== d.id))} className="text-gray-400 hover:text-red-500">
                            <Trash2 size={16} />
                        </button>
                    </div>
                ))}
...
// --- 3. Organization Tab (Workers Only) ---

const OrganizationTab: React.FC<SettingsProps> = ({ workers, setWorkers }) => {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="space-y-4">
            <h3 className="font-semibold text-lg text-slate-700 flex items-center gap-2"><Users size={20}/> Список Сотрудников</h3>
                        ...
                        {workers.map(w => (
                            <tr key={w.id} className="hover:bg-gray-50">
                                ...
                                <td className="p-3 text-right">
                                    <button onClick={() => setWorkers(workers.filter(x => x.id !== w.id))} className="text-gray-400 hover:text-red-500">
                                        <Trash2 size={16}/>
                                    </button>
                                </td>
                            </tr>
